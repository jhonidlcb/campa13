...
export default function Home() {
  ...
  const onSubmit = (data: InsertSupporter) => {
    createSupporter.mutate(data, {
      onSuccess: () => {
        toast({
          title: "¡Gracias por sumarte!",
          description: "Tus datos fueron recibidos. Pronto nos pondremos en contacto.",
          duration: 5000,
        });
        form.reset();
      },
  ...
  return (
    <div className="min-h-screen font-sans bg-slate-50">
      ...
      <section id="hero" className="relative min-h-[90vh] flex items-center pt-20 overflow-hidden bg-primary">
        ...
        <div 
          key={homeContent?.heroImage}
          className="absolute inset-0 z-0 bg-cover bg-center mix-blend-overlay opacity-30"
          style={{ backgroundImage: `url("${homeContent?.heroImage || "https://images.unsplash.com/photo-1540910419892-f0c74b0e8967?q=80&w=2070&auto=format&fit=crop"}")` }}
        />
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full border border-white/20 text-sm font-bold tracking-wider uppercase">
                ...
                {homeContent?.allianceMovement || "Alianza por el Progreso 2026"}
              </div>
              
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-[0.85] text-balance">
                {homeContent ? (
                  homeContent.heroTitle.split(" ").map((word, i) => (
                    <span key={i} className={i % 2 === 1 ? "text-primary bg-accent/20 px-2 rounded-lg" : ""}>{word} </span>
                  ))
                ) : (
                  <div className="h-20 w-full bg-white/10 animate-pulse rounded-lg" />
                )}
              </h1>
              ...
              <div className="pt-8 flex items-center gap-4">
                <div className="flex -space-x-3">
                  ...
                  <div className="w-10 h-10 rounded-full border-2 border-white bg-accent flex items-center justify-center text-[10px] font-bold text-primary shadow-sm z-10">
                    +
                  </div>
                </div>
                <div className="bg-white/10 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/20 shadow-lg group hover:bg-white/20 transition-colors cursor-default">
                  <span className="font-black text-white block text-2xl leading-none tracking-tight">
                    {countData ? countData.count.toLocaleString() : "5"}
                  </span>
                  <span className="text-white/80 text-xs font-bold uppercase tracking-widest">Vecinos ya se sumaron</span>
                </div>
              </div>
            </motion.div>

            <motion.div 
              key={homeContent?.candidateImage || "placeholder"}
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-4 md:px-6">
          <div className="max-w-4xl mx-auto bg-slate-50 rounded-3xl p-8 md:p-12 shadow-xl border border-slate-100 flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="space-y-6">
              ...
              <p className="text-slate-600 leading-relaxed text-lg">
                Como concejal, mi rol es asegurar que cada guaraní del municipio se invierta en los barrios. 
                Legislar no es solo escribir ordenanzas, es defender tus intereses y controlar la gestión 
                para que las soluciones lleguen a tu puerta.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* PROPOSALS SECTION */}
      <section id="propuestas" className="py-24 bg-slate-50 relative overflow-hidden">
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          <SectionHeading 
            title="Mi Compromiso" 
            subtitle="Acciones desde la Junta"
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {dynamicProposals && dynamicProposals.length > 0 ? (
              ...
              <>
                <ProposalCard 
                  title="Iluminación por Barrios"
                  problem="Salud y Seguridad"
                  solution="Ordenanza para el mantenimiento preventivo y ampliación de la red lumínica LED en zonas críticas."
                  icon={<Building2 className="w-full h-full" />}
                  delay={0}
                />
                <ProposalCard 
                  title="Caminos de Todo Tiempo"
                  problem="Infraestructura"
                  solution="Gestión para el empedrado y mantenimiento de caminos vecinales en las compañías."
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="text-white space-y-6">
              ...
              <p className="text-lg text-white/90 mb-8">
                ...
                y a nuestra ciudad. Dejanos tus datos para coordinar el trabajo en tu barrio.
              </p>
              
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-white/10 backdrop-blur-sm p-6 rounded-2xl border border-white/10">
                  <div className="text-3xl font-bold text-white mb-1">Unidos</div>
                  <div className="text-sm text-white/80">Por el Pueblo</div>
                </div>
                <div className="bg-white/10 backdrop-blur-sm p-6 rounded-2xl border border-white/10">
                  <div className="text-3xl font-bold text-white mb-1">
                    {countData ? countData.count.toLocaleString() : "..."}
                  </div>
                  <div className="text-sm text-white/80">Colorados Sumados</div>
                </div>
              </div>
            </div>

            <div className="bg-white rounded-3xl p-8 shadow-2xl">
              <h3 className="text-2xl font-bold text-slate-900 mb-6">Quiero ser parte</h3>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="Ej: 0981 123 456" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="Tu barrio o compañía" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  >
                    {createSupporter.isPending ? "Enviando..." : "Enviar mis datos"}
                  </Button>
                </form>
              </Form>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-12 mb-12">
            <div>
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className="text-white text-xl font-bold">Alianza 2026</span>
              </div>
              <p className="mb-6">
